...
loadData().then(({achievements,methods})=>{ const id=queryId(), m=methods[id]; if(!m) throw new Error('Method not found.'); document.title=`${m.name} | DBD Farming Guide`;
 ...
 const variants=(m.variants||[]).map(v=>`<div class="variant"><strong>${escapeHtml(v.name)}</strong><div>${escapeHtml(v.text)}</div>${v.needsCheck?'<div class="variant-check">Variant worth checking before treating it as the main recommendation.</div>':''}</div>`).join('');
 ...
 <div class="method-section"><h2>How to do it</h2><ol class="method-steps">${(m.steps||[]).map(s=>`<li>${escapeHtml(s)}</li>`).join('')}</ol></div>
 ${m.notes?.length?`<div class="method-section"><h2>Notes</h2><ul class="bullet-list">${m.notes.map(n=>`<li>${escapeHtml(n)}</li>`).join('')}</ul></div>`:''}
 ${variants?`<div class="method-section"><h2>Variants / extra ideas</h2>${variants}</div>`:''}
 </section>`;
